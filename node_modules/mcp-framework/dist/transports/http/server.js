...
import { DEFAULT_HTTP_STREAM_CONFIG } from "./types.js";
...
export class HttpStreamTransport extends AbstractTransport {
    ...
    _activeSseConnections = new Set();
    _requestStreamMap = new Map();
    _pendingBatches = new Map();
    _eventCounter = 0;
    _globalMessageStore = new Map();
    _pruneInterval;
    constructor(config = {}) {
        super();
        this._config = {
    ...
    getCorsHeaders(req, includeMaxAge = false) {
        ...
        return new Promise((resolve, reject) => {
            this._server = createServer(async (req, res) => {
                if (this._config.headers)
                    setResponseHeaders(res, this._config.headers);
                try {
                    ...
                    if (!res.headersSent)
                        res.writeHead(error.statusCode || 500, { "Content-Type": JSON_CONTENT_TYPE });
                    if (!res.writableEnded)
                        res.end(JSON.stringify({ jsonrpc: "2.0", id: error.requestId || null, error: { code: error.code || -32000, message: error.message || "Internal Server Error", data: error.data } }));
                }
            });
    ...
    async handleRequest(req, res) {
        ...
        if (url.pathname !== this._config.endpoint) {
            ...
            res.end("Not Found");
            logger.warn(`Request to unknown path: ${url.pathname}`);
            return;
        }
        try {
            switch (req.method) {
    ...
    async handlePost(req, res) {
        ...
        if (!reqContentTypeHeader)
            throw this.httpError(415, 'Unsupported Media Type: Content-Type header missing');
        const reqContentType = contentType.parse(reqContentTypeHeader);
        if (reqContentType.type !== JSON_CONTENT_TYPE)
            throw this.httpError(415, `Unsupported Media Type: Content-Type must be ${JSON_CONTENT_TYPE}`);
        ...
        try {
            ...
            if (messages.length === 0)
                throw new Error('Empty JSON array received');
        ...
        if (clientRequests.length === 0) {
            ...
            if (responseMode === 'batch') {
                ...
                if (newSessionId) {
                    ...
                    this.completeBatchResponse = (state) => {
    ...
    handleBatchTimeout(res) {
        ...
        if (batchState && !batchState.isCompleted) {
            ...
            const finalResponse = { jsonrpc: "2.0", id: null, error: errorJson };
            if (!res.headersSent)
                res.writeHead(504, { 'Content-Type': JSON_CONTENT_TYPE });
            if (!res.writableEnded)
                res.end(JSON.stringify(finalResponse));
            this._pendingBatches.delete(res);
        }
    }
    completeBatchResponse(batchState) {
        ...
        if (!batchState.res.writableEnded)
            batchState.res.end(body);
    ...
    async handleDelete(req, res) {
        ...
        if (!this._config.session.enabled)
            throw this.httpError(405, 'Method Not Allowed: Sessions are disabled');
        if (!this._config.session.allowClientTermination)
            throw this.httpError(405, 'Method Not Allowed: Client session termination is disabled');
        const sessionIdHeader = getRequestHeader(req.headers, this._config.session.headerName);
    ...
    setupSSEConnection(req, res, sessionId, lastEventId, additionalHeaders = {}, isPostConnection = false) {
        ...
        if (res.socket) {
            ...
            logger.debug(`Optimized socket for SSE stream ${streamId}`);
        }
        else {
            logger.warn(`Could not access socket for SSE stream ${streamId} to optimize.`);
        }
        ...
        const cleanupHandler = (reason) => { if (connection.pingInterval) {
            clearInterval(connection.pingInterval);
            connection.pingInterval = undefined;
        } this.cleanupConnection(connection, reason); };
    ...
    cleanupConnection(connection, reason) {
        if (!this._activeSseConnections.has(connection))
            return;
        ...
        const requestIdsToRemove = [];
        this._requestStreamMap.forEach((conn, reqId) => { if (conn === connection)
        ...
        if (requestIdsToRemove.length > 0)
            logger.debug(`Removed ${requestIdsToRemove.length} request associations for closed stream ${streamId}`);
        if (connection.res && !connection.res.writableEnded) {
            try {
                connection.res.end();
            }
            catch (e) {
                logger.warn(`Error ending response stream ${streamId}: ${e.message}`);
            }
        }
     ...
     */
    checkAndCloseCompletedPostConnection(connection) {
        ...
        this._pendingBatches.forEach(batchState => {
            ...
            if (batchState.res && !batchState.res.writableEnded) {
                try {
                    ...
                    logger.warn(`Error ending response during cleanup: ${error.message}`);
                }
            }
        });
    ...
    sendPing(connection) {
        ...
        try {
            ...
            if (this._activeSseConnections.has(connection)) {
                this.cleanupConnection(connection, `Ping error: ${error.message}`);
            }
        }
    }
    async handleAuthentication(req, res, context, session, requestId) {
        const provider = this._config.auth?.provider;
        if (!provider) {
            logger.debug(`Auth skipped for ${context}: No provider.`);
            return true;
        }
        logger.debug(`Attempting auth for ${context} via ${provider.constructor.name}`);
        let authResult;
        try {
            authResult = await provider.authenticate(req);
        }
        catch (error) {
            logger.error(`Auth provider error for ${context}: ${error.message}`);
            throw this.httpError(500, "Authentication provider error", -32001, error, requestId);
        }
        if (!authResult) {
            const errDet = provider.getAuthError?.() || DEFAULT_AUTH_ERROR;
            logger.warn(`Auth failed for ${context}: ${errDet.message}`);
            throw this.httpError(errDet.status, errDet.message, -32002, undefined, requestId);
        }
        logger.info(`Auth successful for ${context}.`);
        return typeof authResult === 'object' ? authResult : true;
    }
    validateSession(sessionIdHeader, req, isMandatory, requestId) {
        if (!this._config.session.enabled)
            throw this.httpError(500, "Internal Server Error: Session validation called when sessions disabled", -32003, undefined, requestId);
    ...
    async handleResumption(connection, lastEventId, sessionId) {
    ...
    httpError(statusCode, message, code = -32000, data, requestId) {
    ...
    async close() {
        ...
        return new Promise((resolve, reject) => {
            if (this._server) {
                ...
                server.close((err) => { clearTimeout(timeout); if (err) {
                ...
                logger.debug("HTTP server already closed.");
                resolve();
            }
        });
    }
    isRunning() { return Boolean(this._server?.listening); }
    storeMessage(message, sessionId, eventId) {
        if (!this._config.resumability.enabled)
            return;
        const timestamp = Date.now();
        const messageEntry = { eventId, message, timestamp };
    ...
    getMessagesAfterEvent(sessionId, lastEventId) {
